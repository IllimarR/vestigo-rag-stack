...
- ✓ **Architecture diagrams** — `docs/architecture.md` now ships two ASCII diagrams: a runtime-topology view of external actors → HTTP gateways → orchestrators → contracts → backing systems, and a dependency-direction view showing arrows pointing into the `contracts` package. The diagrams are self-contained references, and the modularity-proof section links to the dependency view explicitly.
- ✓ **HTTP-retry hardening for wire-bound providers** — shared helper at `services/llm/application/_retry.py` retries `httpx.TransportError` plus 429/5xx with exponential backoff (0.5s → 1s → 2s, capped at three attempts). All three wire-bound unary paths use it: `OpenAIHttpEmbeddingProvider.embed`, `OpenAIHttpGenerationProvider.generate`, `AnthropicGenerationProvider.generate`. Streaming paths are deliberately *not* retried — once tokens start flowing the stream cannot be resumed and replaying would risk double-billing; SSE callers handle stream errors at the client level. The decision is encoded inline in both `generate_stream` methods.
- ✓ **Corpus validation** — run locally against a representative institutional corpus (ingest → retrieve → generate paths exercised end-to-end against the thesis validation criteria). The corpus and transcripts are deliberately *not* checked into this public repository because the documents would expose internal data of the public institution they were drawn from; the reproduction recipe in `docs/deployment.md` lets a reviewer rerun the same flow on their own corpus.
